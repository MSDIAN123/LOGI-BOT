...
from Logi import INSPECTOR, LOGGER, REQUESTER, dispatcher
from Logi.Handlers.validation import user_admin, user_not_admin, is_user_admin
from Logi.Plugins.Admin.log_channel import loggable
from Logi.Database import reporting_sql as sql
    ...
    if chat.type == chat.PRIVATE:
        if len(args) >= 1:
            if args[0] in ("yes", "on"):
                ...
                msg.reply_text(
                    "Turned on reporting! You'll be notified whenever anLogi reports something."
                )
